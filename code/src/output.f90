...
INTEGER               :: indi, indj
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
CHARACTER(LEN=255) :: FileNameOct_AF_Err_CELLS
CHARACTER(LEN=255) :: FileNameOct_AF_Err_INTERFACES
REAL,ALLOCATABLE   :: Err_CELLS_NVisu(:,:,:)
REAL,ALLOCATABLE   :: Err_INTERFACES_NVisu(:,:,:)
#endif
!-------------------------------------------------------------------------------!
...
#endif
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
ALLOCATE(Err_CELLS_NVisu(1:nVar+1,1:nElemsX,1:nElemsY))
ALLOCATE(Err_INTERFACES_NVisu(1:nVar+1,1:nElemsX+1,1:nElemsY))
#endif
...
#endif
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
FileNameOct_AF_Err_CELLS      = TRIM("CELLS_") // TRIM(FileNameOct)
FileNameOct_AF_Err_INTERFACES = TRIM("INTERFACES_X_") // TRIM(FileNameOct)
#endif
...
#endif
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
    Err_CELLS_NVisu(1:nVar,ii,jj)=Errors_CELLS_AF(1:nVar,ii,jj)
#endif
  END DO
END DO
...
#endif
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
    Err_INTERFACES_NVisu(1:nVar,ii,jj)=Errors_INTERFACES_AF(1:nVar,ii,jj)
#endif
  END DO
END DO
...
IF (WhichOutput .EQ. 1) THEN
  ...
  CALL WriteDataToDisk_OCTAVE(&
        ...
        Err_CELLS_NVisu,             &
        OutputTime,          &
        TRIM(FileNameOct_AF_Err_CELLS),      &
        "FV2D")
  CALL WriteDataToDisk_OCTAVE(&
        nVar,                &
        (/nElemsX+1,nElemsY/), &
        'XY',                &
        MeshBary_X(1:2,1:nElemsX+1,1:nElemsY),&
        VarNameVisu,         &
        Err_INTERFACES_NVisu,             &
        OutputTime,          &
        TRIM(FileNameOct_AF_Err_INTERFACES),      &
        "FV2D")
#endif
