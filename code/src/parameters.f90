...
USE MOD_FiniteVolume2D_vars,ONLY: scaling_factor_relaxed_dt
...
!*------------------------------------------
  ...
  CASE(450) ! Double rarefaction problem - PAMPA algorithm
    TEnd    = 0.6
    Gmm     = 1.4
    nElemsX = 400 
    nElemsY = 5
  ...
  CASE(451) ! Lblanc problem - PAMPA algorithm
    TEnd    = 6.0
    Gmm     = 1.4
    nElemsX = 800 
    nElemsY = 5
...
WhichRiemannSolver = 2                                     !* 0 Rusanov, -1 CU, 1 LDCU,   2 LDCU new antidiffusion, 3 Exact
#if defined(ACTIVEFLUX) || defined(CENTEREDPRIMITIVE)
WhichRiemannSolverPrimitiveSystem =1                       !* 0 Rusanov, -1 CU, 1 LDPCCU
#endif
ReconstructedVariable =1 !*0=Conserved, 1=Characteristic, 2=Primitive, -1=Characteristic Alex' and Shaoshuai
IF (ReconstructedVariable .NE. 1) THEN
    PRINT*, "RUN WITH RECONSTRUCTION OF CHAR ONLY"
    STOP
END IF
#ifdef OTHERPOSTPROCESSING
    PRINT*, "RUN WITH ORIGINAL PP ONLY"
    STOP
#endif
...
#endif
#ifdef OTHERPOSTPROCESSING
PRINT*, "Non-local post-processing considering the neighboring cell averages"
PRINT*, "I WANT YOU TO RUN WITH ORIGINAL PP"
STOP
#endif
#ifdef POSTPROCESSINGATEACHSTEP
PRINT*, "Post-processing at each stage"
PRINT*, "I do not want you to run with this configuration"
STOP
#endif

#endif

#ifdef ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX
PRINT*, "1D-Shock detection based on U-V"
#endif
...
SELECT CASE (timescheme)
  CASE(11,12,21,22,-2,-12,-22,-32,-102)   
...
#ifdef MULTIFLUID
IF (ReconstructedVariable .EQ. 1) THEN
  PRINT*, "Reconstruction of characteristic variables must be checked before using with multifluid"
  STOP
