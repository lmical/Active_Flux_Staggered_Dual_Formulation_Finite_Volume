...
\begin{frame}{Euler equations}
	...
	\begin{columns}
		\begin{column}{0.45\textwidth}
			...
			\begin{minipage}[t]{1.0\linewidth}
				\scalebox{0.9}{ % Adjust this scale as needed
					\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black, title=\textcolor{blue}{\textbf{...in conservation form}}, fonttitle=\scriptsize]
						...
						\begin{align*}
							...
							p=\left(\textcolor{magenta}{\gamma}-1\right)\left\lbrack \right. E-&\frac{1}{2}\rho v^2\left. \right\rbrack
						\end{align*}
						\vspace{-0.1cm}
					\end{tcolorbox}
				}
				...
				\scalebox{0.9}{ % Adjust this scale as needed
					\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black, title=\textcolor{red}{\textbf{...in primitive form}}, fonttitle=\scriptsize]
						...
						\begin{align*}
							...
							\textcolor{black}{\B(\uvec{V}):=}&\textcolor{black}{\begin{pmatrix}0&0&0\\0&0&-\dfrac{1}{\rho}\\0&-\textcolor{magenta}{\gamma} p&-v\end{pmatrix}}
						\end{align*}
						\vspace{-0.6cm}
					\end{tcolorbox}
				}
	...
	\begin{columns}
			...
			\begin{minipage}[t]{1.0\linewidth}
				\scalebox{0.9}{ % Adjust this scale as needed
					\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black, title=\textcolor{black}{\textbf{Multifluid Setting (Challenging)}}, fonttitle=\scriptsize]
						...
						\scriptsize
						\begin{align*}
							\textcolor{magenta}{\gamma}:=\begin{cases}
		...
		\begin{column}{0.45\textwidth}
			% The image with larger labels
			\vspace{-0.1cm}

			\begin{center}
				\scalebox{0.7}{ % Adjust this scale as needed
					\begin{adjustbox}{scale=0.45}
						\begin{tikzpicture}[scale=2.5]
							\def\innerScale{1.2}
							\fill[green!50] (0, 0) rectangle (6, 3.8);
							\draw[thick] (0, 0) rectangle (6, 3.8);
							\begin{scope}
								\draw[thick, smooth, fill=yellow!50] 
								plot [smooth cycle, tension=1] coordinates {
									(2, 1.2)
									(2 + 0.5*\innerScale, 1.2 + 0.8*\innerScale)
									(3 * \innerScale, 1.6 * \innerScale)
									(2.8 * \innerScale, 0.8 * \innerScale)
									(2, 0.4 * \innerScale)
									(1.2 * \innerScale, 0.8 * \innerScale)
									(1 * \innerScale, 1.6 * \innerScale)
									(1.7 * \innerScale, 2 * \innerScale)
								};
							\end{scope}
							\node at (4, 2.5) {\Huge $\phi\leq0$}; % Increase size
							\node at (2.4, 0.9) {\Huge $\phi>0$}; % Increase size
						\end{tikzpicture}
					\end{adjustbox}
				}
			\end{center}
	...
	% Insert the footnotes without extra numbering issues
	\hspace{-40em}\footnote{\tiny A. Chertock, S. Chu, A. Kurganov, 2021, Hybrid Multifluid Algorithms Based on the Path-Conservative Central-Upwind Scheme.}
...
\begin{frame}{Shock--Bubble Interaction}
	\centering
	\begin{minipage}[t]{0.48\textwidth} % Adjust width to 48% for better side-by-side fit
		\centering
		\textbf{Helium} % Display the caption as bold text
		% First set of images
		\foreach \index in {5, 16, 31} {
			\includegraphics[width=1.0\textwidth, trim=20 20 0 0, clip]{results/simulation_frames_helium/frame_\index.png}
		}
	\end{minipage}
	\hfill % Add horizontal space between minipages
	\begin{minipage}[t]{0.48\textwidth} % Adjust width to 48% for symmetry
		\centering
		\textbf{R-22} % Display the caption as bold text
		% Second set of images
		\foreach \index in {5, 17, 31} {
			\includegraphics[width=1.0\textwidth, trim=20 20 0 0, clip]{results/simulation_frames_r22/frame_\index.png}
		}
	\end{minipage}
\end{frame}
...
\centering \large
Thank you...\\
and see you at the poster session!
