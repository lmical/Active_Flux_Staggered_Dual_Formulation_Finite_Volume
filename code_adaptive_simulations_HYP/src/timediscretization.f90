...
USE MOD_FiniteVolume2D     ,ONLY: Compute_Errors_AF
#endif
!-------------------------------------------------------------------------------!
IMPLICIT NONE
!-------------------------------------------------------------------------------!
...
CALL ComputeTransitionMatricesConservedInput(U( 1:nVar,-nGhosts:nElemsX+nGhosts+1,-nGhosts:nElemsY+nGhosts+1),  &  !*Vector
  ...
  IF (iStep .LT. NRelaxedTimesteps+1) THEN
    ...
    dt_min=SQRT(MESH_DX(1)*MESH_DX(2))*scaling_factor_relaxed_dt
    PRINT*, "...at dt", dt_min
  END IF
...
!*==============================
!*NB: CALL BEFORE POSTPROCESSING
!*==============================
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
CALL Compute_Errors_AF(t+dt)
#endif
