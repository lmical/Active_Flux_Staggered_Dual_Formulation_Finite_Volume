...
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
INTERFACE Compute_Errors_AF
  MODULE PROCEDURE Compute_Errors_AF
END INTERFACE
#endif
...
#if defined(CENTEREDPRIMITIVEIMEX)
...
#if defined(ACTIVEFLUX) || defined(CENTEREDPRIMITIVE)
#if defined(IMEX) || defined(IMEXMOMENTUM) || defined(CENTEREDPRIMITIVEIMEX)
...
#endif
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
USE MOD_FiniteVolume2D_vars,ONLY: Errors_CELLS_AF
USE MOD_FiniteVolume2D_vars,ONLY: Errors_INTERFACES_AF
#endif
...
ALLOCATE( WCt(1:nVar,1:nElemsX,1:nElemsY))
...
ALLOCATE(Wa_WC(1:MStepsMax,1:nVar,1:nElemsX,1:nElemsY))
ALLOCATE(Wp_WC(1:MStepsMax,1:nVar,1:nElemsX,1:nElemsY))
ALLOCATE(FWp_WC(1:MStepsMax,1:nVar,1:nElemsX,1:nElemsY))
#endif
...
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
ALLOCATE( Errors_CELLS_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1,-nGhosts:nElemsY+nGhosts+1)) 
ALLOCATE( Errors_INTERFACES_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1+1,-nGhosts:nElemsY+nGhosts+1)) !*NB: +1 in X direction
#endif
...
#endif
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
DEALLOCATE(Errors_CELLS_AF     )
DEALLOCATE(Errors_INTERFACES_AF)
#endif
...
DO jj=vbYs,vbYe
  DO ii=vbXs,vbXe
    Wt(1:nVar,ii,jj) =  - (FX(1:nVar,ii+0,jj+0)-FX(1:nVar,ii-1,jj+0)-BX_Vol(1:nVar,ii,jj)-BX_Sur(1:nVar,ii-1,jj+0,2)+BX_Sur(1:nVar,ii+0,jj+0,1))/Mesh_DX(1) &
...
#ifdef ACTIVEFLUX
#if defined(IMEX) || defined(IMEXMOMENTUM)
SUBROUTINE FVTimeDerivativeConservedOnly(t)
!-------------------------------------------------------------------------------!
USE MOD_Equation,       ONLY: BoundaryConditions
USE MOD_Equation,       ONLY: SourceTerms
USE MOD_Reconstruction, ONLY: ReconstructionX
...
#ifndef PRIMITIVEONLY
CALL BoundaryConditions(t) !*ALERT, NOT NECESSARY IN PRACTICE
...
#if(1==1)
IF ( (InitialCondition .EQ. (-302)) .OR. (InitialCondition .EQ. (-303)) .OR. (InitialCondition .EQ. (350)) ) THEN
...
!===============================================================================!
#if defined(ACTIVEFLUX) && defined(ONEDIMENSIONALSHOCKDETECTORACTIVEFLUX)
SUBROUTINE Compute_Errors_AF(t)
!-------------------------------------------------------------------------------!
USE MOD_FiniteVolume2D_vars,ONLY: U
USE MOD_FiniteVolume2D_vars,ONLY: nVar
USE MOD_FiniteVolume2D_vars,ONLY: nElemsX
...
INTEGER            :: ii, jj, indn, indi, indj
CHARACTER(LEN=255) :: ErrorMessage
REAL               :: W_temp(1:nVar), Error_temp(1:nVar), weight
REAL               :: Errors_CELLS_AF_before_smoothing(1:nVar,-nGhosts:nElemsX+nGhosts+1,-nGhosts:nElemsY+nGhosts+1)
REAL               :: Errors_INTERFACES_AF_before_smoothing(1:nVar,-nGhosts:nElemsX+nGhosts+1+1,-nGhosts:nElemsY+nGhosts+1) !*NB: +1 in X direction
REAL               :: QtempM(1:nVar)
REAL               :: QtempP(1:nVar)
REAL               :: Qtemp(1:nVar), Vtemp(1:nVar)
REAL               :: Qaverage(1:nVar)
...
Errors_CELLS_AF      = 0.0
Errors_INTERFACES_AF = 0.0

Errors_CELLS_AF_before_smoothing      = 0.0
Errors_INTERFACES_AF_before_smoothing = 0.0
...
DO jj=1,nElemsY
  DO ii=1,nElemsX
    CALL PrimToCons(0.5*(W_X(1:nVar,ii,jj)+W_X(1:nVar,ii+1,jj)),W_temp(1:nVar))
    Errors_CELLS_AF_before_smoothing(1:nVar,ii,jj)=ABS(U(1:nVar,ii,jj)-W_temp(1:nVar))    
  END DO
END DO
...
DO jj=1,nElemsY
  DO ii=1,nElemsX+1 !*NB:+1 in X direction
    ...
    CALL PrimToCons(W_X(1:nVar,ii,jj),Vtemp(1:nVar)) 
    Errors_INTERFACES_AF_before_smoothing(1:nVar,ii,jj)=ABS(Qtemp(1:nVar)-Vtemp(1:nVar))
...
DO jj=1,nElemsY
  DO ii=1,nElemsX
    ...
    !*If we are at the boundary, only consider the cell itself, as corners may not be available
    IF ( (ii .EQ. 1) .OR. (ii .EQ. nElemsX) .OR. (jj .EQ. 1) .OR. (jj .EQ. nElemsY)  ) THEN
      Errors_CELLS_AF(1:nVar,ii,jj)=Errors_CELLS_AF_before_smoothing(1:nVar,ii,jj)
    ELSE
      ...
      indj=jj
      weight=4.0/9.0
      Error_temp(1:nVar)=Error_temp(1:nVar)+Errors_CELLS_AF_before_smoothing(1:nVar,indi,indj)*weight
      ...
      Errors_CELLS_AF(1:nVar,ii,jj)=Error_temp(1:nVar)      
...
DO jj=1,nElemsY
  DO ii=1,nElemsX+1
    ...
    !*If we are at the boundary, only consider the cell itself, as corners may not be available
    IF ( (ii .EQ. 1) .OR. (ii .EQ. nElemsX+1) .OR. (jj .EQ. 1) .OR. (jj .EQ. nElemsY)  ) THEN
      Errors_INTERFACES_AF(1:nVar,ii,jj)=Errors_INTERFACES_AF_before_smoothing(1:nVar,ii,jj)
    ELSE
      ...
      indj=jj
      weight=4.0/9.0
      Error_temp(1:nVar)=Error_temp(1:nVar)+Errors_INTERFACES_AF_before_smoothing(1:nVar,indi,indj)*weight
      ...
      Errors_INTERFACES_AF(1:nVar,ii,jj)=Error_temp(1:nVar)      
...
#else
Errors_CELLS_AF(1:nVar,:,:) = Errors_CELLS_AF_before_smoothing(1:nVar,:,:)
Errors_INTERFACES_AF(1:nVar,:,:) = Errors_INTERFACES_AF_before_smoothing(1:nVar,:,:)
#endif
...
!*MAKE SURE A 1D TEST IS REALLY A 1D TEST
#if(1==1)
IF ( (InitialCondition .EQ. (-302)) .OR. (InitialCondition .EQ. (-303)) .OR. (InitialCondition .EQ. (-304)) .OR. (InitialCondition .EQ. (-305)) .OR. (InitialCondition .EQ. (350)) ) THEN
  Errors_CELLS_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1,1)       = Errors_CELLS_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1,2)
  Errors_CELLS_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1,nElemsY) = Errors_CELLS_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1,nElemsY-1)
  Errors_INTERFACES_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1+1,1)       = Errors_INTERFACES_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1+1,2)
  Errors_INTERFACES_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1+1,nElemsY) = Errors_INTERFACES_AF(1:nVar,-nGhosts:nElemsX+nGhosts+1+1,nElemsY-1)
END IF
#endif
#endif

!-------------------------------------------------------------------------------!
END SUBROUTINE Compute_Errors_AF
#endif
