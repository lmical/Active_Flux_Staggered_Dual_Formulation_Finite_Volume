...
# Define file paths
mesh_x_file = os.path.join(folder, "W_Y_MESH_CoordinateX.dat")
mesh_y_file = os.path.join(folder, "W_Y_MESH_CoordinateY.dat")
solution_file = os.path.join(folder, "W_Y_SOLUTION_OCT_0000000.2500000.dat")  # Specific time step file
...
ax.plot_surface(X, Y, RO, color='blue', alpha=1.0, edgecolor='none', linewidth=0.5)
ax.set_title(r"$\rho$")
ax.set_xlabel('X')
ax.set_ylabel('Y')
...
plt.savefig("sod2d_V_Y_3d_density_without_grids.pdf", format="pdf", bbox_inches="tight", transparent=True)
# plt.show()
