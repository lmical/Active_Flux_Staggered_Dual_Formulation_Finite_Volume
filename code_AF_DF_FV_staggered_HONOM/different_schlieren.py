...
def plotting(folder_name, file_name, levels):
    ...
    other_schlieren=True
    if other_schlieren:
        K=0
        VORT=np.log(1.0+VORT)/np.log(10)
    else:
        maxgrad = np.max(VORT)
        K = 80
    ...
    if other_schlieren:
        if word != None:
            cont_vort = ax.contourf(X, Y, VORT, cmap=word, levels=levels)    
        else:
            cont_vort = ax.contourf(X, Y, VORT, cmap='Greys', levels=levels)
    else:
        if word != None:
            cont_vort = ax.contourf(X, Y, VORT, cmap=word+'_r', levels=levels)    
        else:
            cont_vort = ax.contourf(X, Y, VORT, cmap='Greys_r', levels=levels)
    cont_vort.set_clim(0.05, 2.4)
