...
#endif
#ifdef STAGGERED
CHARACTER(LEN=255) :: FileNameOct_WS
CHARACTER(LEN=255) :: FileNameOct_WS_ERR
REAL,ALLOCATABLE   :: WS_NVisu(:,:,:)
REAL,ALLOCATABLE   :: WS_ERR_NVisu(:,:,:)
#endif
...
#endif
#ifdef STAGGERED
ALLOCATE(WS_NVisu(1:nVar+1,1:nElemsX+1,1:nElemsY+1))
ALLOCATE(WS_ERR_NVisu(1:nVar+1,1:nElemsX+1,1:nElemsY+1))
#endif
...
#endif
#ifdef STAGGERED
FileNameOct_WS     = TRIM("WS_") // TRIM(FileNameOct)
#endif
...
PRINT*, "--------------------------"
DO iVar=1,nVar
  PRINT*, "Maximum and Minimum of primitive variable WS", iVar, "is", MAXVAL(WS(iVar,1:nElemsX+1,1:nElemsY+1)),  MINVAL(WS(iVar,1:nElemsX+1,1:nElemsY+1))
END DO
PRINT*, "--------------------------"
#endif
#if defined(ACTIVEFLUX) || defined(CENTEREDPRIMITIVE) 
...
DO jj=1,nElemsY+1
  DO ii=1,nElemsX+1
    WS_NVisu(1:nVar,ii,jj) = WS(1:nVar,ii,jj)
    WS_NVisu(nVar+1,ii,jj) = 0.0
#if MOMENTUMINPRIMITIVEVARIABLES
    WS_NVisu(2:3,ii,jj) = WS(2:3,ii,jj)/WS(1,ii,jj)
#endif
  END DO
END DO
#endif
...
IF (WhichOutput .EQ. 1) THEN
  ...
  CALL WriteDataToDisk_OCTAVE(&
        ...
        (/nElemsX+1,nElemsY+1/), &
        'XY',                &
        MeshBary_S(1:2,1:nElemsX+1,1:nElemsY+1),&
        VarNameVisu_W,       &
        WS_NVisu,             &
        OutputTime,          &
        TRIM(FileNameOct_WS),      &
...
#ifdef STAGGERED
Filename = TRIM("WS_MESH_CoordinateX.dat")
WRITE(*,'(A22,1X,A)') &
  "Writing DATA to Disk: ", "OCTAVE ASCII"//" => "//TRIM(FileName)
OPEN(UNIT_FILE, FILE=Filename)
WRITE(UNIT_FILE,'(A)') "CoordinateX"
DO ii=1,nElemsX+1 
  WRITE(UNIT_FILE,'(SP,1(ES21.14E2))') MeshBary_S(1,ii,1)
END DO
CLOSE(UNIT_FILE)

Filename = TRIM("WS_MESH_CoordinateY.dat")
WRITE(*,'(A22,1X,A)') &
  "Writing DATA to Disk: ", "OCTAVE ASCII"//" => "//TRIM(FileName)
OPEN(UNIT_FILE, FILE=Filename)
WRITE(UNIT_FILE,'(A)') "CoordinateY"
DO jj=1,nElemsY+1
  WRITE(UNIT_FILE,'(SP,1(ES21.14E2))') MeshBary_S(2,1,jj)
END DO
CLOSE(UNIT_FILE)
#endif
