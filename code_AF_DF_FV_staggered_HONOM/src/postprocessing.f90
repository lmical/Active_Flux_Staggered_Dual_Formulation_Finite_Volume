...
#ifdef STAGGERED
ALLOCATE( L1error(1:nVar) )
ALLOCATE( Wexact(1:nVar,1:nElemsX+1,1:nElemsY+1)) 
...
DO jj = 1,nElemsY+1
  DO ii = 1,nElemsX+1
    DO iGP=1,nGPs
      DO jGP=1,nGPs
        CALL ExactFunction(InitialCondition, tEnd, MeshGP_S(:,ii,jj,iGP,jGP), WtempCons(1:nVar,iGP,jGP))
        CALL ConsToPrim(WtempCons(1:nVar,iGP,jGP),WtempPrim(1:nVar,iGP,jGP))
        Wexact(1:nVar,ii,jj) = Wexact(1:nVar,ii,jj) + WeightsGP(iGP,jGP)* WtempPrim(1:nVar,iGP,jGP)
      END DO
    END DO
    L1error(:) = L1error(:) + ABS( WS(:,ii,jj) - Wexact(:,ii,jj) ) 
  ENDDO
ENDDO

L1error(:) = L1error(:) * MESH_DX(1) * MESH_DX(2)

PRINT*, "Error WS", L1error

FNAME = 'WS_ErrorL1_XXXX_XXXX.dat'
WRITE(FNAME(9+3:12+3),FMT="(I4.4)") nElemsX
WRITE(FNAME(14+3:17+3),FMT="(I4.4)") nElemsY
