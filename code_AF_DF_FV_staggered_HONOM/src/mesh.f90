...
DO jj=1,nElemsY
  DO ii=1,nElemsX
    MeshBary_S(2,ii,jj) = MeshBary(2,ii,jj)-0.5*Mesh_DX(2) !*Staggered y
    MeshBary_S(1,ii,jj) = MeshBary(1,ii,jj)-0.5*Mesh_DX(1) !*Staggered x
  END DO
END DO

!*Last point in X direction skipped
DO jj=1,nElemsY 
  MeshBary_S(2,nElemsX+1,jj) = MeshBary_S(2,nElemsX,jj)              !*Same y
  MeshBary_S(1,nElemsX+1,jj) = MeshBary_S(1,nElemsX,jj)+Mesh_DX(1)   !*Staggered x
END DO

!*Last point in Y direction skipped
DO ii=1,nElemsX+1
  MeshBary_S(2,ii,nElemsY+1) = MeshBary_S(2,ii,nElemsY)+Mesh_DX(2)     !*Staggered y
  MeshBary_S(1,ii,nElemsY+1) = MeshBary_S(1,ii,nElemsY)                !*Same x
END DO
...
DO iGP = 1, nGPs
  DO jGP = 1, nGPs
    DO jj=1,nElemsY+1
      DO ii=1,nElemsX+1
        MeshGP_S(1:nDims,ii,jj,iGP,jGP) = (/ MeshBary_S(1,ii,jj) +quadNodes1D(iGP)*Mesh_DX(1) , MeshBary_S(2,ii,jj)  +quadNodes1D(jGP)*Mesh_DX(2) /)
      END DO
    END DO
  END DO
END DO
