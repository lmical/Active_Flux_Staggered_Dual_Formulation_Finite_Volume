...
def plotting(folder_name, filename, what_plot='all', n_levels=20):
    ...
    print(np.max(P), np.min(P))  # Update to check pressure values

    # Define data for plots based on 'what_plot'
    plot_data = {}
    if what_plot in ['pressure']:
        plot_data['p'] = P
    ...
    # Define subplots based on the 'what_plot' parameter
    axes = {}
    if what_plot == 'pressure':  # Change this to 'pressure'
        axes['p'] = fig.add_subplot(111)  # Single plot for pressure
    elif what_plot == 'density':
    ...
    plt.tight_layout()
    plt.savefig("sod2d_U_contour_pressure.pdf", format="pdf", bbox_inches="tight")
    # plt.show()

if __name__ == '__main__':
    # Change to plot pressure instead of density
    plotting(folder_name, filename, 'pressure', n_levels=14) #50 #70
